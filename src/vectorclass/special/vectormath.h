...
static inline Vec4f atan (Vec4f const & a, Vec4f const & b) {   // inverse tangent of a/b
    float aa[4], bb[4];
    a.store(aa);  b.store(bb);
    return Vec4f(atan2f(aa[0],bb[0]), atan2f(aa[1],bb[1]), atan2f(aa[2],bb[2]), atan2f(aa[3],bb[3]));
}
static inline Vec2d atan (Vec2d const & a, Vec2d const & b) {   // inverse tangent of a/b
    double aa[4], bb[4];
    a.store(aa);  b.store(bb);
...
static inline Vec4f cubic_root (Vec4f const & x) {   // pow(x,1/3)
    return amd_vrs4_cbrtf(x);
}
static inline Vec2d cubic_root (Vec2d const & x) {   // pow(x,1/3)
    return amd_vrd2_cbrt(x);
}

// static inline Vec4f recipr_sqrt (Vec4f const & x) {   // pow(x,-1/2)
// static inline Vec2d recipr_sqrt (Vec2d const & x) {   // pow(x,-1/2)
...
static inline Vec8f cubic_root (Vec8f const & x) {   // pow(x,1/3)
    return Vec8f(cubic_root(x.get_low()), cubic_root(x.get_high()));
}
static inline Vec4d cubic_root (Vec4d const & x) {   // pow(x,1/3)
    return Vec4d(cubic_root(x.get_low()), cubic_root(x.get_high()));
}
...
static inline Vec4f recipr_sqrt (Vec4f const & x) {   // pow(x,-1/2)
    return __svml_invsqrtf4(x);
}
static inline Vec2d recipr_sqrt (Vec2d const & x) {   // pow(x,-1/2)
    return __svml_invsqrt2(x);
}
...
static inline Vec8f recipr_sqrt (Vec8f const & x) {   // pow(x,-1/2)
    return __svml_invsqrtf8(x);
}
static inline Vec4d recipr_sqrt (Vec4d const & x) {   // pow(x,-1/2)
    return __svml_invsqrt4(x);
}
...
static inline Vec8f recipr_sqrt (Vec8f const & x) {   // pow(x,-1/2)
    return Vec8f(recipr_sqrt(x.get_low()), recipr_sqrt(x.get_high()));
}
static inline Vec4d recipr_sqrt (Vec4d const & x) {   // pow(x,-1/2)
    return Vec4d(recipr_sqrt(x.get_low()), recipr_sqrt(x.get_high()));
}
